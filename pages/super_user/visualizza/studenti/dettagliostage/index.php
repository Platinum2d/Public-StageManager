<?php
    ...
    $idscuola = $connessione->query("SELECT id_scuola FROM scuola, classe WHERE scuola_id_scuola = id_scuola AND id_classe = $idclasse")->fetch_assoc()['id_scuola'];
                            ...
                            <?php
                                ...
                                while ($row = $result->fetch_assoc())
                                {
                                    ...
                                    echo "<tr id=\"riga$I\" style=\"text-align : center\"> <td> ".date("d-m-Y", strtotime($row['inizio_stage']))." </td> <td> ".$row['durata_stage']." giorni </td>"
                                         . " <td>"
                                         . "<button id=\"dettagli$I\" style=\"margin : 0px\" onclick=\"openInfo($I, $idclasse, $idclassestage, $idstudente, $id_studente_has_stage, $idanno, $idscuola)\" class=\"btn btn-success btn-sm margin buttonfix\"> <span class=\"glyphicon glyphicon-edit\"></span> Dettagli </button> "
