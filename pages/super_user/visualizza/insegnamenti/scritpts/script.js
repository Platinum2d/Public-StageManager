...
function openInfo(progressiv, nome_classe, id_classe, id_anno)
{
    ...
    $.ajax({
        ...
        data : {'classe' : id_classe, 'anno' : id_anno, 'scuola' : $("#schools").val()},
        cache : false,
...
function updateDocs(id_scuola)
{
    $.ajax({
        type : 'POST',
        url : 'ajaxOpsPerInsegnanti/ajaxClassi.php',
        data : {'scuola' : id_scuola},
        cache : false,
        success : function (xml)
        {
            $("#schoolDocs").find("tbody").html("");
            var I = 0;
            $(xml).find("classi").find("classe").each(function (){
                var id_classe = $(this).find("id").text();
                var nome_classe = $(this).find("nome_classe").text();
                var nome_settore = $(this).find("nome_settore").text();
                var indirizzo_studi = $(this).find("indirizzo_studi").text();
                
                $("#schoolDocs").find("tbody").append("<tr id='riga"+I+"'>\n\
                                                            <td align='center'>"+nome_classe+"</td>\n\
                                                            <td align='center'>"+indirizzo_studi+" "+nome_settore+"</td>\n\
                                                            <td align='center'>\n\
                                                            <button id='info"+I+"' onclick='$(this).prop(\"disabled\", true); openInfo("+I+", \""+nome_classe+"\", "+id_classe+", "+$("#anno").val()+");' class='btn btn-success'><span class='glyphicon glyphicon-info-sign'></span> Visualizza</button>\n\
                                                            </td>\n\
                                                      </tr>");
                I++;
            });
