<?php
    ...
    function checkPrefExistence($pref){
        $connessione = dbConnection("../../../../");
        $query = "SELECT id_preferenza FROM preferenza WHERE nome = '".$connessione->escape_string($pref)."'";
    ...
    echo "<html>";
        ...
        echo "<body>";
        if (is_uploaded_file ( $_FILES ['preffile'] ['tmp_name'] ))
        {
            $fileName = $_FILES ['preffile'] ['name'];
            echo "<h3 style=\"color:green\"> ==== CARICAMENTO EFFETTUATO CON SUCCESSO ==== </h3>";
            $filepath = "../../../../media/loads/";
            if (move_uploaded_file ( $_FILES ['preffile'] ['tmp_name'], $filepath . $fileName)) {
                ...
                $reporterrori = "";
                    ...
                    for ($I=2; $I<$rows;$I++)
                    {
                        ...
                        $exists = checkPrefExistence($nome);
                        if (isset($nome) && !empty($nome))
                        {
                            if(!$exists)
                            {
                                $insertquery = "INSERT INTO preferenza (nome) VALUES ('".$conn->escape_string($nome)."')";
                                if ($conn->query($insertquery))
                                {
                                    echo "Generata la preferenza $nome <br><br>";
                            ...
                            {
                                $reporterrori .= "<br><h3 style=\"color:red\"> ==== PREFERENZA GIA' ESISTENTE ALLA RIGA $I ==== </h3>";
                            }
                        }
                        else
                        {
